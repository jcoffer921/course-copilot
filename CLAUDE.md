...
- Faculty planning (see `docs/superpowers/plans/2026-09-24-faculty-view-excel-import.md`): `UserSettings.role` (`student`/`faculty`) plus `agent.authentication.FacultyPermission`/`is_faculty()` gate a faculty-only surface the same way `PilotOwnerPermission`/`is_pilot_owner()` gates analytics — a faculty account is redirected off the student dashboard/nav entirely, into `/faculty/` (`faculty_dashboard_page`), `/faculty/import/`, and `/faculty/cora/` (`faculty_planner_page`, the actual working workflow). `agent/services/requirements_extraction.py` turns a faculty-uploaded program-requirements `.xlsx` into structured JSON (never written to disk/DB by itself); `agent/services/program_requirements.py` is the one place a confirmed `ProgramRequirement` row gets written, via `POST /api/faculty/requirements/import/` (extract only) and `.../confirm/` (create, or 409-then-overwrite on a `(user, program_name, catalog_year)` collision). `agent/services/academic_planner.py` then holds an AI planning chat grounded against exactly one `ProgramRequirement`, dropping any model-proposed course not present in it; all student-specific state (details, conversation, draft plan) lives only in `request.session["faculty_plan_session"]` via `POST /api/faculty/plan/chat/` and `.../plan/reset/`/`.../plan/export/` (DOCX download, `agent/services/plan_export.py`) — never the database, same rule this doc's stack notes already hold content extraction to. **Known gap:** `/faculty/import/` is currently a non-functional mockup page (see the status doc's "Review findings") — the real, working import is `faculty_planner_page`'s Step 1.
