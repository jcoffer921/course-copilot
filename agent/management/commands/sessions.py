...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["create"]:
            self._create(course_id)
        elif options["list"]:
            self._list(course_id)
        else:
            self._show(course_id, options["show"])

    def _create(self, course_id):
        try:
            session = sessions.create_session(course_id)
    ...
    def _list(self, course_id):
        try:
            summaries = sessions.list_sessions(course_id)
    ...
    def _show(self, course_id, session_id):
        try:
            session = sessions.get_session(course_id, session_id)
